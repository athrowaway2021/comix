...
from .utilities import (list_symbols, convert_jxr_to_tiff, disable_debug_log)
...
class KFX_PDF(object):
    ...
    def convert_image_resources(self):
        ordered_images = self.get_ordered_images(["$286", "$285", "$548", "$284"], include_unreferenced=False, allow_duplicates=True)
        return convert_images_to_pdf_data(ordered_images)

    def get_ordered_images(self, formats, include_unreferenced=True, allow_duplicates=False):
            ...
            if resource_format in formats:
                ...
                if location is not None and location not in image_resources:
                    ...
                    if raw_media is not None:
                        ...
                        image_resources[location] = ImageResource(location, resource_format, raw_media.value)
        ...
        if unused_image_resource_ids and include_unreferenced:
            log.error("Found unreferenced resources: %s" % list_symbols(unused_image_resource_ids))
                ...
                if data_type is IonAnnotation:
                    ...
                    if typ == "$271":
                        ...
                        if resource_name is not None and resource_name not in section_image_resources:
                            ...
                            if allow_duplicates or resource_name not in ordered_image_resources:
