...
from .epub_output import (EPUB_NS_URI, EPUB_TYPE, MATH, qname, SVG, XML_LANG, XML_NS_URI)
...
class KFX_EPUB_Properties(object):
    ...
    def convert_yj_properties(self, yj_properties):
        ...
        for yj_property_name, yj_value in yj_properties.items():
            ...
            if property == "position" and value in ["oeb-page-foot", "oeb-page-head"]:
                property = "display" if self.epub.generate_epub2 and EMIT_OEB_PAGE_PROPS else None
    ...
    def property_value(self, yj_property_name, yj_value, svg=False):
        ...
        if val_type is IonStruct:
            ...
            if yj_property_name == "$173":
                ...
                value = "url(\"%s\")" % urllib.parse.quote(urlrelpath(
                        self.process_external_resource(yj_value).filename, ref_from=self.epub.STYLES_CSS_FILEPATH))
    ...
    def fixup_styles_and_classes(self):
        ...
        for book_part in self.epub.book_parts:
            ...
            for e in body.iter("*"):
                ...
                if "style" in e.attrib:
                    ...
                    if (CVT_DIRECTION_PROPERTY_TO_MARKUP or not self.epub.generate_epub2) and ("direction" in style or "unicode-bidi" in style):
                        unicode_bidi = style.get("unicode-bidi", "normal")

                        has_block = False
                        has_content = e.text
                        for ex in e.iterfind(".//*"):
                            if ex.tag in {"aside", "div", "figure", "h1", "h2", "h3", "h4", "h5", "h6", "iframe", "li", "ol", "table", "td", "ul"}:
                                has_block = True
        ...
        for style_str, count in sorted(style_counts.items(), key=lambda sc: -sc[1]):
            ...
            class_name_prefix = "-".join(sorted(list(class_name_prefixes))) if class_name_prefixes else DEFAULT_CLASS_NAME_PREFIX
    ...
    def update_default_font_and_language(self):
        ...
        if self.epub.language:
            default_language_pattern = re.compile(re.escape(self.epub.language) + "(-.+)?$", re.IGNORECASE)
    ...
    def simplify_styles(self, elem, book_part, inherited_properties, reverse_inheritance, default_ordered_list_value=None):
        ...
        for name, val in list(sty.items()):
            if name in {"padding", "padding-top", "padding-bottom", "padding-left", "padding-right"} and val.startswith("-"):
                ...
                if unit == "rem" and quantity is not None and (self.epub.generate_epub2 or self.epub.GENERATE_EPUB2_COMPATIBLE):
                    if name == "font-size":
        ...
        for child in elem.findall("*"):
            self.simplify_styles(child, book_part, parent_sty, reverse_inheritance, default_ordered_list_value)
    ...
    def create_css_files(self):
        for css_file in sorted(list(self.css_files)):
            if css_file == self.epub.RESET_CSS_FILEPATH:
                self.epub.manifest_resource(self.epub.RESET_CSS_FILEPATH, data=RESET_CSS_DATA.encode("utf-8"), mimetype="text/css")
