...
from .epub_output import (
    ...
    namespace, qname, set_nsmap, SVG, SVG_NAMESPACES, SVG_NS_URI)
...
class KFX_EPUB_Content(object):
    ...
    def process_section(self, section):
        ...
        if self.epub.is_comic:
        ...
        elif (self.epub.is_magazine or self.epub.is_print_replica) and has_conditional_template:
            ...
            for i, page_template in enumerate(page_templates):
                if "$171" not in page_template or self.evaluate_binary_condition(page_template.pop("$171")):
                    ...
                    if layout in ["$325", "$323"]:
                        ...
                        self.add_content(page_template, book_part.html, book_part, self.epub.writing_mode, layout)
            ...
            book_part = self.epub.new_book_part()
            self.process_content(page_templates[-1], book_part.html, book_part, self.epub.writing_mode)
            self.link_css_file(book_part, self.epub.STYLES_CSS_FILEPATH)
            ...
            if len(page_templates) > 1:
                ...
                for i, page_template in enumerate(page_templates[:-1]):
                    ...
                    self.process_content(page_template, body, book_part, self.epub.writing_mode)
                    self.check_empty(page_template, "%s conditional page_template %d" % (self.content_context, i))

        self.pop_context()
        self.check_empty(section, self.content_context)

    def process_page_spread_page_template(self, page_template, section_name, page_spread="", parent_template_id=None):
        ...
        if (page_template["$159"] == "$270" and
            ...
            page_property = left_property if self.epub.page_progression_direction == "ltr" else right_property

            for page_template_ in story.pop("$146", []):
                self.process_page_spread_page_template(page_template_, section_name, page_property, parent_template_id)
            ...
            for page_template_ in story.pop("$146", []):
                self.process_page_spread_page_template(
                        page_template_, section_name, "rendition:page-spread-center", parent_template_id)
                parent_template_id = None
            ...
            self.process_content(page_template, book_part.html, book_part, self.epub.writing_mode)
            self.link_css_file(book_part, self.epub.STYLES_CSS_FILEPATH)
    ...
    def process_content(self, content, parent, book_part, writing_mode, content_layout=None, fixed_height=None, fixed_width=None):
        if self.DEBUG:
        ...
        if content_type == "$269":
            ...
            if top_level and ((is_scale_fit_layout and self.epub.fixed_layout) or layout == "$325"):
            ...
            if layout is None:
                ...
                if "$434" in content:
                    ...
                    if self.epub.is_comic and virtual_panel == "$441":
                        ...
                        log.warning("Unexpected %s container virtual_panel: %s" % (self.epub.book_type, virtual_panel))
        ...
        if min_aspect_ratio is not None and (self.epub.min_aspect_ratio is None or min_aspect_ratio < self.epub.min_aspect_ratio):
            self.epub.min_aspect_ratio = min_aspect_ratio

        max_aspect_ratio = content.pop("$648", None)
        if max_aspect_ratio is not None and (self.epub.max_aspect_ratio is None or max_aspect_ratio > self.epub.max_aspect_ratio):
    ...
    def link_css_file(self, book_part, css_file, css_type="text/css"):
        ...
        link.set("href", urllib.parse.quote(urlrelpath(css_file, ref_from=book_part.filename)))
    ...
    def preformat_spaces(self, elem):
        if elem.tag in {"audio", "iframe", "img", "object", SVG, "video"}:
            ...
            if elem.tag not in {
                    "body", "nav", "aside", "div", "figure", "h1", "h2", "h3", "h4", "h5", "h6", "ul", "ol", "li",
                    "table", "tbody", "thead", "tr", "td", "caption", "br", "colgroup", "col", "hr",
                    IDX_ENTRY, "text", "title", "desc"}:
