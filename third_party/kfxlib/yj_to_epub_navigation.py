...
class KFX_EPUB_Navigation(object):
            ...
            for i, book_navigation in enumerate(book_navigations):
                if book_navigation.get("$178", "") == reading_order_name:
                    ...
                    for nav_container_ in book_navigation.pop("$392"):
                        ...
                        self.process_nav_container(nav_container, nav_container_, reading_order_name)
    ...
    def process_nav_container(self, nav_container, nav_container_name, reading_order_name):
        ...
        if "imports" in nav_container:
            for import_name in nav_container.pop("imports"):
                self.process_nav_container(self.book_data["$391"].pop(import_name), nav_container_name, reading_order_name)
        else:
            for nav_unit_ in nav_container.pop("$247"):
                ...
                if nav_type in {"$212", "$214", "$213"}:
                    self.process_nav_unit(nav_type, nav_unit, self.epub.ncx_toc, nav_container_name, section_name)
    ...
    def process_nav_unit(self, nav_type, nav_unit, ncx_toc, nav_container_name, section_name):
        ...
        for entry in nav_unit.pop("$247", []):
            ...
            self.process_nav_unit(nav_type, nested_nav_unit, nested_toc, nav_container_name, section_name)
        ...
        for anchor_name, elem in self.anchor_elem.items():
            ...
            for book_part in self.epub.book_parts:
                if book_part.html is root:
                    ...
                    self.anchor_uri[anchor_name] = "%s#%s" % (urllib.parse.quote(book_part.filename), elem_id)
                    break
            else:
                ...
                if "id" in e.attrib and not visible_elements_before(e):
                    ...
                    for anchor, a_uri in self.anchor_uri.items():
                        if (a_uri == uri) and (anchor not in self.immovable_anchors):
                            self.anchor_uri[anchor] = urllib.parse.quote(book_part.filename)
                            if self.DEBUG:
