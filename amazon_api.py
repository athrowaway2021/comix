...
def login(email, password, domain = "com", device_id = DEVICE_ID):
    ...
    response_json = requests.post(f"https://api.amazon.{domain}/auth/register", headers=get_auth_headers(domain), json=body).json()
    
    try:
        tokens = {
            "name": hashlib.md5(email.encode()).hexdigest(), # to differentiate tokens from different accounts
            "domain": domain,
            "device_id": device_id,
...
def signed_request(method, url, headers = None, body = None, asin = None, tokens = None, request_id = None, request_type = None):
    """
...
def register_device(tokens = None):
    ...
    resp = requests.Session().send(signed_request("POST", url, headers, body, tokens=tokens))
