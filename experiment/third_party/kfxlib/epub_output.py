...
from .utilities import (EPUB2_ALT_MIMETYPES, MIMETYPE_OF_EXT, make_unique_name, urlrelpath)
...
class BookPart(OPFProperties):
    def __init__(self, filename, part_index, html, opf_properties=set(), linear=None, omit=False, idref=None):
...
class EPUB_Output(object):
    ...
    def set_book_type(self, book_type):
        ...
        self.is_children = self.is_comic = self.is_magazine = self.is_print_replica = False
    ...
    def generate_epub(self):
        ...
        if self.fixed_layout and (not self.is_print_replica) and (self.original_height is None or self.original_width is None):
    ...
    def new_book_part(self, filename=None, opf_properties=set(), linear=True, omit=False, html=None, idref=None):
    ...
    def compare_fixed_layout_viewports(self):
                ...
                if head is not None:
                    for meta in head.iterfind("meta"):
                        if meta.get("name") == "viewport":
                            ...
                            if mw and mh:
                                viewport_count[(int(mw.group(1)), int(mh.group(1)))] += 1
        ...
        if len(viewport_count) == 0:
            ...
            if self.original_width != best_width or self.original_height != best_height:
                log.info("Largest viewport is not the most common: %s" % (
                        ", ".join(["%dw x %dh (%d)" % (fw, fh, ct) for (fw, fh), ct in viewports_by_size])))
            elif len(viewports_by_count) > 1 and REPORT_CONFLICTING_VIEWPORTS:
                log.warning("Conflicting viewport sizes (best %dw x %dh): %s" % (
                        best_width, best_height, ", ".join(["%dw x %dh (%d)" % (fw, fh, ct) for (fw, fh), ct in viewports_by_count])))
            ...
            for e in body.iter("div"):
                if len(e.attrib) == 0:
                    ...
                    if len(parent) == 1 and not parent.text:
                        if parent.tag in {
                                "aside", "div", "figure", "h1", "h2", "h3", "h4", "h5", "h6", "li", "td", "caption",
                                IDX_ENTRY}:
                            e.tag = TEMP_TAG
                            ...
                            for child in e.iterfind("*"):
                                if (child.tag not in {
                                        ...
                                        "ol", "table", "ul", IDX_ENTRY} or child.tail):
                                    break
                            else:
                                e.tag = TEMP_TAG
    ...
    def beautify_html(self, book_part):
        ...
        for e in [html] + html.findall("*") + head.findall("*") + body.findall("*"):
            ...
            if e.tag in {
                    "head", "title", "link", "meta", "style", "body", "aside", "div", "figure",
                    "h1", "h2", "h3", "h4", "h5", "h6", "hr", "nav", "table", "ul", "ol",
                    IDX_ENTRY}:
                e.tail = (e.tail or "") + "\n"
            ...
            if e.tag == "div" and e.get("id", "").startswith("amzn_master_range_"):
                for ee in e.iterfind("*"):
                    if ee.tag in {"aside", "div", "figure", "h1", "h2", "h3", "h4", "h5", "h6",
                                  "hr", "table", "ul", "ol", IDX_ENTRY} and not ee.tail:
                        ee.tail = "\n"
    ...
    def create_opf(self):
        ...
        for manifest_entry in sorted(self.manifest, key=lambda m: m.filename):
            ...
            if manifest_entry.external:
                ...
                href = urlrelpath(urllib.parse.quote(manifest_entry.filename), ref_from=self.OPF_FILEPATH)
