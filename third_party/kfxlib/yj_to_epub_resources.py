...
class KFX_EPUB_Resources(object):
    def get_external_resource(self, resource_name, ignore_variants=False):
        ...
        if "$636" in resource:
            ...
            with disable_debug_log():
                ...
                for y, row in enumerate(col):
                    ...
                    bottom_padding = resource_height - tile_height * len(col) if y == len(col) - 1 else tile_padding

                    for x, location in enumerate(row):
                        left_padding = 0 if x == 0 else tile_padding
                        right_padding = resource_width - tile_width * len(row) if x == len(row) - 1 else tile_padding
                        ...
                        if tile_raw_media is not None:
                            ...
                            if twidth != tile_width + left_padding + right_padding or theight != tile_height + top_padding + bottom_padding:
                                log.error("Resource %s tile %d, %d size (%d, %d) does not have expected padding %d of (%d, %d) for %s" % (
                                    resource_name, x, y, twidth, theight, tile_padding, tile_width, tile_height, resource_name))
                ...
                if full_image.size != (resource_width, resource_height):
                    log.error("Resource %s combined tiled image size is (%d, %d) but should be (%d, %d) for %s" % (
                            resource_name, full_image.size[0], full_image.size[1], resource_width, resource_height, resource_name))
                ...
                while True:
                    ...
                    outfile = io.BytesIO()
                    full_image.save(outfile, "jpeg" if extension == ".jpg" else extension[1:], quality=quality)
                ...
                if (best_size_diff * 100) // separate_tiles_size > TILE_SIZE_REPORT_PERCENTAGE or True:
                    log.warning("Image resource %s has %d tiles with total size %d combined into image of size %d quality %d" % (
                        resource_name, tile_count, separate_tiles_size, len(raw_media), best_quality))
        ...
        resource_obj = self.resource_cache[resource_name] = Obj(
                    raw_media=raw_media, filename=filename, extension=extension, format=format, mime=mime, location=location,
    ...
    def process_external_resource(self, resource_name, save=True, process_referred=False, save_referred=False,
        ...
        if save and self.save_resources and resource_obj.raw_media is not None is resource_obj.manifest_entry is None:
            ...
            while filename in self.epub.oebps_files:
                if self.epub.oebps_files[filename].binary_data == resource_obj.raw_media:
